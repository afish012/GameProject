...
boolean collides(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
  float u1 = ((x4-x3)*(y1-y3) - (y4-y3)*(x1-x3)) / ((y4-y3)*(x2-x1) - (x4-x3)*(y2-y1)); //find the point along line 1 where the lines intersect
  float u2 = ((x2-x1)*(y1-y3) - (y2-y1)*(x1-x3)) / ((y4-y3)*(x2-x1) - (x4-x3)*(y2-y1)); //find the point along line 2 where the lines intersect
  // if u1 and u2 are between 0-1, lines are colliding
...
PVector collidePoint(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
  float u1 = ((x4-x3)*(y1-y3) - (y4-y3)*(x1-x3)) / ((y4-y3)*(x2-x1) - (x4-x3)*(y2-y1)); //find the point along line 1 where the lines intersect
  return new PVector(x1 + (u1 * (x2-x1)), y1 + (u1 * (y2-y1))); //return the location of the intersect
}
